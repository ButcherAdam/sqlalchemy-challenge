...
def tobs():
    ...
    recent_date = dt.date(2017,8,23)
    last_year = recent_date - dt.timedelta(days=365)
    # Query all tobs data for most active station
    results = session.query(Measurement.date, Measurement.tobs, Measurement.prcp).filter(Measurement.date >= last_year).filter(Measurement.station == 'USC00519281').order_by(Measurement.date).all()
